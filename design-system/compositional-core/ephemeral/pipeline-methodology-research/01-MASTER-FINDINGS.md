...
- CD: "Can N patterns coexist with M constraints?" (integration testing) — e.g., "Can 3 heavy component types honor the velocity rule while CRESCENDO builds through an F-Pattern to Bento Grid transition?"
...
TC derives from content to tension to metaphor to structure. Explorations derive from structural hypothesis to instantiation to test.
...
Each exploration inherited ALL previous findings. The knowledge was not compressed — it grew.
...
- **v1 to v2 to v3 enrichment waves** (43-agent OD re-enrichment)
...
During the exploration era, "context" was not a CONSTRAINT to be managed — it was a PRODUCT to be grown.
...
- 337 research findings to 7 vocabulary files (48:1 compression)
- 88 stage findings to 20 mechanism names (4.4:1 compression)
- Implementation-mapped plans to bare names with no maps
- Navigation architecture to flat file dump
...
This is the architectural principle that made the curation layer work. Knowledge did not sit in scratchpad directories. Every finding was compiled into a package that a builder consumed. The pipeline has no equivalent.
...
Builders encountered provenance headers when reading dependency HTML files (HANDOFF-AD-TO-CD explicitly says "READ the validated HTML artifacts"). But they did not TRAVERSE the graph. The headers CONTEXTUALIZED knowledge but did not DISCOVER it.

Builders PRODUCED provenance headers. Starting with OD builds, builders created properly-formatted inline threading headers as part of their output. This shows the provenance format was part of their cognitive process, not external documentation.
...
Builder (gets same 7 files every time + brief)
        |
PA Evaluation then Fix Cycles (no findings production)
```
...
3. **No findings production.** Pipeline builds do not discover new things that feed back.
...
- CSS growth: 418 to 603 lines across 6 explorations
...
- Documented v1 to v2 to v3 enrichment (43-agent, 7-wave)
- Reference density: DD-001 ~5 refs to OD-006 ~50+ refs (10:1 growth)
...
- TC = bottom-up (content to tension to metaphor to structure)
- Explorations = top-down (structural hypothesis to instantiation to test)
- Missing category: STRUCTURAL PROPOSITIONS
...
- Builders DID produce their own headers

### Agent 7: Enrichment Wave Investigation
- Documented 43-agent 7-wave OD re-enrichment producing 42 new EXT findings
- ACCUMULATED-IDENTITY evolution: v1 (542 lines) to v1.1 (576) to v2 (518, denser)
...
AD had NO equivalent re-enrichment cycle. Result: 28 findings vs OD 17, BUT each finding 3.6x less densely threaded. 47 "dark matter" items. Critical files left in scratchpad.
...
3. Reframing: "knowledge-growth problem reframed as context-management problem"
...
- 03-PIPELINE-GAP-ANALYSIS.md — What pipeline lacks and why
- 04-GENERATIVE-QUESTIONS.md — All 24 questions + TC comparison
- 05-PROVENANCE-MECHANISM.md — Provenance and navigation system

---

## 14. The Tripod: Three Instruments as One System (ADDED 2026-03-01)
...
- Read ALL applicable research (R-5 combination theory, etc.)
- JUDGED which findings applied to each specific builder
- Mapped findings to implementation plans
...
- **Without #1 (master execution prompt):** Builder has a great package but no protocol. No sequencing, no finding-production requirement. Builder might read linearly instead of using as reference while building.

- **Without #2 (research package):** Master prompt says "apply R-5 findings" but builder must find them across 39 scattered documents. Builder drowns in raw material.

- **Without #3 (curation process):** Package exists but is static, not curated for THIS assignment. Lists findings that may not apply. No implementation mapping. Same package every time (= pipeline's 7 vocabulary files).

### The Pipeline's State

The pipeline has the SKELETON of Instrument 1 (a builder prompt) but removed the SUBSTANCE that made it effective:
- Instrument 1: Exists (compose skill builder prompt) but points to static files, not curated packages
- Instrument 2: MISSING — every builder gets the same 7 files, no per-build package
