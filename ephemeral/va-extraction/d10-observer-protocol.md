...
#### Category 2: BEFORE Checklist Verification (IMPROVE Protocol)

| Check ID | What | How | Severity |
|----------|------|-----|----------|
| OBS-05 | BEFORE checklist answered before IMPROVE builder spawn | If IMPROVE occurs: read tracker for IMPROVE section fields, verify all filled BEFORE IMPROVE builder spawn timestamp | BLOCKING |
| OBS-06 | IMPROVE builder is different agent | Compare builder agent description/spawn context between Cycle 0 and IMPROVE. If identical prompt text detected, flag | BLOCKING |
| OBS-07 | IMPROVE builder received correct inputs | Read the IMPROVE builder's input files (conviction + artistic + RESIDUAL + brief + Cycle 0 HTML). Verify: NO gate scores, NO threshold numbers, NO fix lists, NO diagnostic vocabulary ("fix", "repair", "address", "correct") | BLOCKING |
| OBS-08 | RESIDUAL artifact exists | Grep Cycle 0 HTML for `<!-- RESIDUAL:` comment. Must be >= 50 chars and contain >= 3 of 4 required sections | SIGNIFICANT |
| OBS-09 | Post-IMPROVE re-audit happened | Verify `-improve` suffixed PA reports exist in output directory (9 reports). Verify `-improve` gate results exist | BLOCKING |

**Verification method:** The observer reads output files. For OBS-07 (isolation), it reads the IMPROVE builder's prompt/input files and searches for prohibited content. This is the highest-value check — IMPROVE isolation violation is the most consequential orchestrator error.
...
| **Pass R** | After IMPROVE completes (if applicable) | OBS-05 through OBS-09, then repeat Pass 2-4 for IMPROVE artifacts |
...
- IMPROVE builder received gate scores or diagnostic vocabulary (OBS-07)
...
- IMPROVE re-audit did not happen (OBS-09)
...
- SKIP: NN (not applicable, e.g., IMPROVE checks when no IMPROVE occurred)
...
Against a baseline of 80-105 min and 15 agents, this is a 12-15% time increase and 6.7% agent increase. The ROI is high: one prevented IMPROVE isolation violation saves ~45 min + 11 agent spawns.
...
- OBS-21 [BLOCKING]: Sample 2 random auditor reports. Search for pipeline vocabulary:
  ...
  "brief assembler", "IMPROVE", "gate runner", "PA-05 >= 3.5".
...
### Pass R — After IMPROVE (if applicable)
Trigger: Any file matching {BUILD_DIR}/*-improve* exists

Checks:
- OBS-05 [BLOCKING]: IMPROVE section in tracker has fields filled BEFORE IMPROVE builder
  spawn timestamp. The BEFORE checklist must be answered BEFORE the builder runs.
- OBS-06 [BLOCKING]: Compare Cycle 0 builder spawn context with IMPROVE builder spawn context.
...
- OBS-07 [BLOCKING]: Read any IMPROVE builder input files or the IMPROVE brief.
  ...
  ANY hit = IMPROVE isolation violation = STOP.
- OBS-08 [SIGNIFICANT]: Grep Cycle 0 HTML for <!-- RESIDUAL: comment. Must be >= 50 chars
  and contain >= 3 of: "preserved", "changed", "next", "trade-off".
- OBS-09 [BLOCKING]: Exactly 9 -improve PA auditor reports exist. Gate results with
  -improve suffix exist. Full Phase 3 re-audit must have occurred.
- Then repeat Pass 2, 3, 4 checks on IMPROVE-suffixed artifacts.
...
After the pipeline completes (RELEASE, POLISH, or HALT), write your comprehensive
...
| `OBSERVER-PASS-R.md` | After IMPROVE (if applicable) | IMPROVE verification results |
...
**Mitigation:** This is an inherent limitation. The observer verifies ARTIFACTS, not INTENT. It catches the most consequential violations (IMPROVE isolation, fresh-eyes, screenshot completeness) even if it cannot catch subtle judgment errors. Process compliance is a proxy for decision quality, not a replacement.
...
1. **IMPROVE isolation violations** — The orchestrator might accidentally include gate scores in the IMPROVE prompt. It would self-report "isolation verified: YES" because it believes the prompt is clean. The observer READS the prompt and checks for prohibited content.
