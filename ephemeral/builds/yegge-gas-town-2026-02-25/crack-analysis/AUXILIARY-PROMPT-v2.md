# Pipeline Integrity Protocol — Drop-in Fragment

> Paste this into any session that modifies pipeline artifacts.
> It replaces the 9-section voluntary checklist with 3 structural gates + 4 binary audits.

---

## AFTER every file edit, RUN this verification procedure

This is not optional guidance. This is a procedure you execute. If you skip it, the changes you made will silently break downstream pipeline stages.

### Gate 1: Reference Integrity Scan

For every file you modified, run this check:
...
**What this catches:** Cross-file reference rot (the #1 source of silent pipeline failures across 4 builds). Gate IDs, file names, section numbers, and step references go stale every time a file is split, merged, or reorganized.
...
If MISSING_slots > 0, add the missing slots to the template. A recipe step without a template slot will be lost in 100% of builds. This has been proven across every pipeline run.

If you modified `artifact-tc-brief-template.md` (the template):

```
1. Verify the template does NOT exceed 250 lines (current: ~223)
2. Each new slot must be fillable in <= 15 lines by the assembler
```

Brief length past ~350 lines degrades builder compliance from ~85% to ~72%.
...
Compliance drops from ~95% to ~72% when agent input exceeds these thresholds. Every line you add reduces compliance with every other line the agent must follow. This is not theoretical — it is measured across 6 pipeline runs.
...
If RATIO < 1.0, you added voluntary rules. Voluntary rules achieve 0-67% compliance. Convert to code, template slots, or gate checks — or accept that the rule will be followed in fewer than 2 out of 3 builds.
...
If NET DELTA > +50 and none of the additions are code-enforced: you increased the specification surface area without increasing enforcement. The pipeline already has 885 items with 6% enforcement coverage. Adding more unenforced spec makes the problem worse.
...
If INSTANCES > 0, convert each to a binary rule or delete it. "Target 8+" produces 5. "Minimum 8, gate fails below 8" produces 8. Binary rules achieve 100% compliance. Judgment rules achieve 0-65%.
...
| Gate coverage (GR-48) | 100% of REQUIRED gates | Never achieved; make automatic |

---

## What this prompt does NOT do

It does not tell you HOW to design pipeline changes. It does not contain creative guidance, compositional intelligence theory, or quality philosophy. Those belong in the recipe, the mechanism catalog, and the skill files.

This prompt does ONE thing: it forces you to verify that your changes do not silently break the pipeline's existing integrity. The verification is mechanical, binary, and takes < 10 minutes. If you cannot pass these checks, your changes are not ready to merge.
