...
<head>
  ...
  <style>
    /* ═══════════════════════════════════════════════════════════════════════
       ROOT TOKENS — Complete page architecture
       Zone naming is concept-based (refinery metaphor):
       perimeter = gate/qualification, credentials = authority/trust,
       clearing = insight/revelation, floor = dense processing,
       vault = memory systems, console = implementation
       ═══════════════════════════════════════════════════════════════════════ */
    :root {
      ...
      /* ZONE BACKGROUNDS — page-specific, concept-based
         R3: density rhythm → zone differentiation supports density arc
         Conviction brief: "same physics, different purpose" → industrial warmth with soot */
      --zone-perimeter: #F5EDE3;     /* 245,237,227 — industrial warmth, gritty testing */
      --zone-credentials: #FAF5ED;   /* 250,245,237 — trust-building, the founder's office */
      --zone-clearing: #FEF9F5;      /* 254,249,245 — clean breathing, insight landing */
      --zone-floor: #EDE5D8;         /* 237,229,216 — dense processing (chunks 2-4) */
      --zone-vault: #F8F3EB;         /* 248,243,235 — memory systems (chunk 3) */
      --zone-console: #FAF5ED;       /* 250,245,237 — implementation (chunk 4-5) */

      /* Standard zone aliases for shared components */
      --color-zone-sparse: var(--zone-clearing);
      --color-zone-dense: #FAFAF8;
      --color-zone-breathing: var(--zone-credentials);

      /* TYPOGRAPHY — LOCKED TRINITY */
      --font-display: 'Instrument Serif', Georgia, serif;
      --font-body: 'Inter', system-ui, sans-serif;
      --font-mono: 'JetBrains Mono', 'SF Mono', monospace;

      /* TYPE SCALE — ~1.4x ratio between levels
         R5: typographic scale jumping → hierarchy through size, not decoration */
      --type-page: 2.75rem;
      ...
      /* BORDER SYSTEM — 3 categories, never 2px */
      ...
      /* SOLID OFFSET — depth without box-shadow */
      --offset-x: 4px;
      --offset-y: 4px;
      --offset-color: #1A1A1A;
    }
    ...
    *, *::before, *::after {
    ...
    /* Soul Piece #1 — sharp edges, always */
    ...
    *:focus-visible {
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       HEADER — Dark background, industrial weight
       Mechanism #13: Dark Header + 3px Primary Border
       Conviction brief: "factory district" → dark header extends authority
       ═══════════════════════════════════════════════════════════════════════ */
    .page-header {
    ...
    .page-header__provenance span + span::before {
      content: "·";
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       ZONES — Each zone is a distinct room with its own atmosphere
       R3: zone background differentiation → background + spacing + borders
       shift together at each boundary (multi-channel coherence)
       ═══════════════════════════════════════════════════════════════════════ */
    .zone {
    ...
    .zone--perimeter {
      background: var(--zone-perimeter);
    }

    .zone--credentials {
      background: var(--zone-credentials);
    }

    .zone--clearing {
      background: var(--zone-clearing);
    }

    .zone--floor {
      background: var(--zone-floor);
    }

    .zone--vault {
      background: var(--zone-vault);
    }

    .zone--console {
      background: var(--zone-console);
    }

    /* ═══════════════════════════════════════════════════════════════════════
       SECTION STYLES — Shared structure for all content sections
       ═══════════════════════════════════════════════════════════════════════ */
    .section {
      max-width: 960px;
      margin: 0 auto;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       TABLE OF CONTENTS — Factory manifest board
       R4: F-pattern for text-heavy scanning → left-aligned list
       Conviction brief: TOC as navigation, not decoration
       ═══════════════════════════════════════════════════════════════════════ */
    .toc {
      background: var(--zone-credentials);
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       COMPLEXITY LADDER — Vertical axis element
       Conviction brief: "a literal ascending structure that should feel
       like looking up at something tall"
       Instrument Serif for ladder labels (per brief)
       R3: dense/sparse alternation → active rung breaks pattern
       ═══════════════════════════════════════════════════════════════════════ */
    .ladder {
      max-width: 640px;
    ...
    /* R5: border-weight gradient → 4px active, 3px standard, 1px faded */
    .ladder__rung--active {
      border-left: 4px solid var(--color-primary);
      background: #FEF5F4; /* 254,245,244 — warm alarm tint */
    ...
    .ladder__rung--active .ladder__level {
      color: var(--color-primary);
      font-weight: 500;
    }

    /* Instrument Serif for ladder labels (per conviction brief) */
    .ladder__name {
      font-family: var(--font-display);
      font-size: var(--type-body);
      font-style: italic;
    ...
    .ladder__rung--active .ladder__name {
      font-weight: 700;
    }
    ...
    .ladder__rung--active + .ladder__connector {
      background: var(--color-primary);
      width: 4px;
    }

    /* ═══════════════════════════════════════════════════════════════════════
       STATUS BLOCKS — Current state / After document
       R5: dense/sparse alternation within section
       ═══════════════════════════════════════════════════════════════════════ */
    .status-grid {
      display: grid;
      grid-template-columns: 1fr 1fr;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       GATE — Prerequisites checkpoint
       Conviction brief: "checkboxes that feel consequential"
       R5: border-left semantic signal → red left border = alarm/gate
       ═══════════════════════════════════════════════════════════════════════ */
    .gate {
      border: 3px solid var(--color-border);
      border-left: 4px solid var(--color-primary);
    ...
    .gate__item--warning .gate__checkbox {
      border-color: var(--color-primary);
    }

    /* ═══════════════════════════════════════════════════════════════════════
       COMPLEXITY BADGE — Level indicator
       ═══════════════════════════════════════════════════════════════════════ */
    .complexity-badge {
      display: inline-flex;
      align-items: center;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       DATA TABLES — Bio, comparisons, metrics
       Mechanism #18: Data Table (Mono Headers + Border Categories)
       ═══════════════════════════════════════════════════════════════════════ */
    .data-table {
      width: 100%;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       CREDENTIAL LIST — Compact authority markers
       ═══════════════════════════════════════════════════════════════════════ */
    .credential-list {
      list-style: none;
      padding: 0;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       LINK LIST — Public presence, compact
       ═══════════════════════════════════════════════════════════════════════ */
    .link-list {
      list-style: none;
      padding: 0;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       PULL QUOTE — Raw, profane, earned voice
       Conviction brief: "profane, chaotic, survival-focused" quotes
       Red border = alarm/attention (per world description)
       Instrument Serif = editorial authority
       ═══════════════════════════════════════════════════════════════════════ */
    .pull-quote {
      border-left: 4px solid var(--color-primary);
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       PARADIGM SHIFT — Side-by-side comparison
       Conviction brief: "asymmetric layout — the contrast doing the teaching"
       Old approach is muted, new approach is forward/warm
       ═══════════════════════════════════════════════════════════════════════ */
    .paradigm-shift {
      display: grid;
      grid-template-columns: 1fr 1fr;
    ...
    .paradigm-shift__column {
      padding: var(--space-6) var(--space-8);
    }

    /* Old way: muted, constrained — feels like the past */
    .paradigm-shift__column--old {
      background: #F0E8DC; /* 240,232,220 — earthier, heavier */
      border-right: 3px solid var(--color-border);
    }

    /* New way: clean, warm — feels like the future */
    .paradigm-shift__column--new {
      background: var(--color-zone-dense);
    }

    .paradigm-shift__header {
      font-family: var(--font-mono);
      font-size: var(--type-meta);
      font-weight: 600;
      text-transform: uppercase;
    ...
    .paradigm-shift__column--old .paradigm-shift__header {
      color: var(--color-text-secondary);
    }

    .paradigm-shift__column--new .paradigm-shift__header {
      color: var(--color-primary);
    }

    .paradigm-shift__step {
      padding: var(--space-3) 0;
      font-size: var(--type-body);
      line-height: 1.5;
    }

    .paradigm-shift__step + .paradigm-shift__step {
      border-top: 1px solid var(--color-border-subtle);
    }

    .paradigm-shift__step:last-child {
      font-weight: 600;
    }

    .paradigm-shift__connector {
      font-family: var(--font-mono);
      font-size: var(--type-meta);
      color: var(--color-text-secondary);
      text-align: center;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       IDEA COMPILER BLOCK — Prominent concept display
       ═══════════════════════════════════════════════════════════════════════ */
    .idea-compiler {
      border: 3px solid var(--color-border);
    ...
    .idea-compiler__value {
      font-size: var(--type-body);
    }

    .idea-compiler__statement {
      font-family: var(--font-display);
      font-style: italic;
      font-size: var(--type-body-lg);
      max-width: 60ch;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       REASON LIST — Compact reference list with markers
       ═══════════════════════════════════════════════════════════════════════ */
    .reason-list {
      list-style: none;
      padding: 0;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       VIBE PRINCIPLES — Compact principle list
       ═══════════════════════════════════════════════════════════════════════ */
    .principle-list {
      list-style: none;
      padding: 0;
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       CALLOUT FAMILY — 2-zone structure, 4px left border
       Soul Piece #3 — callout structure is inviolable
       Mechanism #2: 2-Zone Component DNA
       Mechanism #10: Border-Left Semantic Signaling
       ═══════════════════════════════════════════════════════════════════════ */
    .callout {
    ...
    .callout--gotcha .callout__label { color: var(--color-primary); }
    ...
    /* Zone-modulated callout density (per Phase 2 Step 2.4)
       Opening/clearing zones: generous padding
       Dense/floor zones: compressed, heavier borders */
    .zone--perimeter .callout,
    .zone--clearing .callout {
      padding: var(--space-5) var(--space-6);
    }
    ...
    /* ═══════════════════════════════════════════════════════════════════════
       ZONE TRANSITIONS
       CD-006 calibration: Smooth (48px + 1px), Bridge (64px + breathing),
       Breathing (80px + 3px)
       Applied per conviction brief zone boundaries
       ═══════════════════════════════════════════════════════════════════════ */

    /* Smooth: within same zone, different intensity */
    .transition--smooth {
      border-top: 1px solid var(--color-border);
    }

    /* Breathing: cross-zone boundary, major cognitive shift
       Conviction brief: Perimeter → Revelation = "the reader feels the click" */
    ...
    @media print {
      ...
      .code-block { background: #F5F5F5; color: black; border-color: #CCCCCC; }
    }
  </style>
</head>
...
<body>
  ...
  <!-- ═══════════════════════════════════════════════════════════════════════
       HEADER — Dark, industrial, the refinery entrance
       Mechanism #13: Dark Header + 3px Primary Border
       Conviction brief: "KortAI's factory district — same physics, different purpose"
       ═══════════════════════════════════════════════════════════════════════ -->
  <header class="page-header" role="banner">
    <div class="page-header__inner">
      <div class="page-header__meta">
        <span class="page-header__badge">Level 7</span>
        <span>Complexity Ladder</span>
        <span>Complete Extraction</span>
      </div>
      <h1 class="page-header__title">Steve Yegge and Gas Town</h1>
      <p class="page-header__epigraph">The agent factory architect building team-scale productivity for solo developers.</p>
      <div class="page-header__provenance">
  ...
  <main id="main-content">
    ...
    <!-- ═══════════════════════════════════════════════════════════════════════
         TABLE OF CONTENTS — Factory manifest
         The TOC sits in the credentials zone (warm breathing background)
         because it functions as a manifest board — a record of everything
         inside the refinery.
         R4: F-pattern for left-aligned scanning
         ═══════════════════════════════════════════════════════════════════════ -->
    <div class="zone zone--credentials" style="padding-top: var(--space-12); padding-bottom: var(--space-10);">
      <nav class="toc" aria-label="Table of Contents">
        <h2 class="toc__title">Contents</h2>
        <ol class="toc__list">
          <li class="toc__item"><a href="#you-are-here" class="toc__link"><span class="toc__number">00</span> You Are Here</a></li>
          <li class="toc__item"><a href="#who-is-steve-yegge" class="toc__link"><span class="toc__number">01</span> Who is Steve Yegge?</a></li>
          <li class="toc__item"><a href="#gas-town-mental-model" class="toc__link"><span class="toc__number">02</span> The Gas Town Mental Model</a></li>
          <li class="toc__item"><a href="#8-roles-architecture" class="toc__link"><span class="toc__number">03</span> The 8 Roles Architecture</a></li>
          <li class="toc__item"><a href="#beads-memory-system" class="toc__link"><span class="toc__number">04</span> Beads: The Memory System</a></li>
          <li class="toc__item"><a href="#6-waves-of-ai" class="toc__link"><span class="toc__number">05</span> The 6 Waves of AI</a></li>
          <li class="toc__item"><a href="#core-principles" class="toc__link"><span class="toc__number">06</span> Core Principles</a></li>
          <li class="toc__item"><a href="#implementation-guide" class="toc__link"><span class="toc__number">07</span> Implementation Guide</a></li>
          <li class="toc__item"><a href="#checkpoints-verification" class="toc__link"><span class="toc__number">08</span> Checkpoints and Verification</a></li>
          <li class="toc__item"><a href="#troubleshooting" class="toc__link"><span class="toc__number">09</span> Troubleshooting</a></li>
          <li class="toc__item"><a href="#when-to-use" class="toc__link"><span class="toc__number">10</span> When to Use Gas Town</a></li>
          <li class="toc__item"><a href="#quotes-key-statements" class="toc__link"><span class="toc__number">11</span> Quotes and Key Statements</a></li>
          <li class="toc__item"><a href="#comparison-patterns" class="toc__link"><span class="toc__number">12</span> Comparison with Other Patterns</a></li>
          <li class="toc__item"><a href="#sources" class="toc__link"><span class="toc__number">13</span> Sources</a></li>
        </ol>
      </nav>
    </div>
    ...
    <!-- ═══════════════════════════════════════════════════════════════════════
         ZONE 1: THE PERIMETER FENCE
         Sections: You Are Here + Who is Steve Yegge
         Conviction brief: "This zone is deliberately narrow. It qualifies
         the reader before granting entry."
         Density: LOW spatial, HIGH semantic
         Background: warm industrial (#F5EDE3)
         ═══════════════════════════════════════════════════════════════════════ -->

    <!-- ─────────────────────────────────────────────────────────────────────
         SECTION 0: YOU ARE HERE
         The reader feels *tested*. The complexity ladder places them.
         The prerequisites list confronts them.
         ───────────────────────────────────────────────────────────────────── -->
    <div class="zone zone--perimeter" id="you-are-here">
      <section class="section" aria-label="You Are Here">
        <div class="section__meta">Section 00 · Perimeter</div>
        <h2 class="section__title">You Are Here</h2>

        <!-- The Complexity Ladder
             Conviction brief: "a vertical axis element — a literal ascending
             structure that should feel like looking up at something tall.
             Tight, constrained, high editorial authority. Instrument Serif
             for the ladder labels."
             R3: dense/sparse alternation → active rung breaks pattern
             R5: border-weight gradient → 4px active, 3px standard -->
        <div class="ladder" role="figure" aria-label="Complexity Ladder showing 8 levels from single session to Gas Town Factory">
          <div class="ladder__header">The Complexity Ladder</div>
          <div class="ladder__stack">
            <div class="ladder__rung ladder__rung--active" aria-current="true">
              <span class="ladder__level">Level 7:</span>
              <span class="ladder__name">Gas Town Factory</span>
              <span class="ladder__tag">← You are learning this</span>
            </div>
            <div class="ladder__connector"></div>
            <div class="ladder__rung">
              <span class="ladder__level">Level 6:</span>
              <span class="ladder__name">Parallel Swarms + Worktrees</span>
            </div>
            <div class="ladder__connector"></div>
            <div class="ladder__rung">
              <span class="ladder__level">Level 5:</span>
              <span class="ladder__name">Multi-Agent Orchestration (CC Mirror)</span>
            </div>
            <div class="ladder__connector"></div>
            <div class="ladder__rung">
              <span class="ladder__level">Level 4:</span>
              <span class="ladder__name">PRD-Driven Loops (Full Ralph)</span>
            </div>
            <div class="ladder__connector"></div>
            <div class="ladder__rung">
              <span class="ladder__level">Level 3:</span>
              <span class="ladder__name">Simple Loops (Basic Ralph)</span>
            </div>
            <div class="ladder__connector"></div>
            <div class="ladder__rung">
              <span class="ladder__level">Level 2:</span>
              <span class="ladder__name">Subagents + Hooks</span>
            </div>
            <div class="ladder__connector"></div>
            <div class="ladder__rung">
              <span class="ladder__level">Level 1:</span>
              <span class="ladder__name">CLAUDE.md + Slash Commands</span>
            </div>
            <div class="ladder__connector"></div>
            <div class="ladder__rung ladder__rung--faded">
              <span class="ladder__level">Level 0:</span>
              <span class="ladder__name">Single Interactive Session</span>
            </div>
          </div>
        </div>

        <!-- Current state + After this document — side by side
             R5: paradigm shift comparison → two-column status grid
             creates compact density within a LOW spatial zone -->
        <div class="status-grid">
          <div class="status-block">
            <h3 class="status-block__title">Your current state:</h3>
            <p class="status-block__intro">You're a developer who has:</p>
            <ul class="status-block__list">
          ...
          <div class="status-block">
            <h3 class="status-block__title">After this document, you will:</h3>
            <ul class="status-block__list">
        ...
        <!-- Prerequisites — the gate
             Conviction brief: "checkboxes that feel consequential"
             Red left border = alarm. These are serious requirements.
             The budget item ($50-200/day) gets warning treatment. -->
        <div class="gate">
          <div class="gate__label">Prerequisites</div>
          <ul class="gate__list">
            <li class="gate__item">
              <span class="gate__checkbox"></span>
              Stage 7+ on Yegge's 8-Stage Dev Evolution
            </li>
            <li class="gate__item">
              <span class="gate__checkbox"></span>
            ...
            <li class="gate__item gate__item--warning">
              <span class="gate__checkbox"></span>
              Budget: $50-200/day for API costs
            </li>
            <li class="gate__item">
        ...
        <div class="complexity-badge">
          ...
          <span class="complexity-badge__value">Level 7</span>
          <span>on the complexity ladder (highest)</span>
        </div>
    ...
    <!-- ─────────────────────────────────────────────────────────────────────
         SMOOTH TRANSITION: You Are Here → Who is Steve Yegge
         Same zone (perimeter fence), shifting from testing to trust.
         1px separator (CD-CONV smooth type).
         ───────────────────────────────────────────────────────────────────── -->


    <!-- ─────────────────────────────────────────────────────────────────────
         SECTION 1: WHO IS STEVE YEGGE?
         Conviction brief: "The reader feels *trust building*. Yegge's
         credentials transfer conviction. The pull quotes hit: raw,
         profane, earned."
         Zone: credentials (within perimeter) — warmer, the founder's office
         Density: COMPACT — table-dense, quote-punctuated
         ───────────────────────────────────────────────────────────────────── -->
    <div class="zone zone--credentials transition--smooth" id="who-is-steve-yegge">
      <section class="section" aria-label="Who is Steve Yegge">
        <div class="section__meta">Section 01 · Authority</div>
        <h2 class="section__title">Who is Steve Yegge?</h2>
        ...
        <!-- Bio table — compact authority block
             Mechanism #18: Data Table -->
        ...
        <!-- Credential list with red dash markers
             These are authority claims, each backed by evidence -->
        ...
        <ul class="link-list">
          <li>
            <span class="link-list__label">Twitter/X:</span>
            <span class="link-list__value">@steve_yegge (primary announcements)</span>
          </li>
          <li>
            <span class="link-list__label">Medium:</span>
            <span class="link-list__value">steve-yegge.medium.com (deep philosophical pieces)</span>
          </li>
          <li>
            <span class="link-list__label">GitHub:</span>
            <span class="link-list__value">github.com/steveyegge (Gas Town, Beads, MCP Agent Mail)</span>
          </li>
          <li>
            <span class="link-list__label">Podcasts:</span>
            <span class="link-list__value">Changelog, O'Reilly Radar</span>
          </li>
        </ul>
        ...
        <!-- Pull quote — raw, earned voice
             Red border = attention. Instrument Serif = authority.
             The profanity is preserved because it is earned language. -->
        <blockquote class="pull-quote">
          <p class="pull-quote__text">"Work becomes fluid, an uncountable that you sling around freely, like slopping shiny fish into wooden barrels at the docks."</p>
        </blockquote>
    ...
    <!-- ═══════════════════════════════════════════════════════════════════════
         BREATHING TRANSITION: Perimeter Fence → Revelation
         Zone 1 → Zone 2. Major cognitive shift.
         "Should I be here?" → "The click — the mental model restructuring."
         3px structural border + 80px gap (breathing type).
         ═══════════════════════════════════════════════════════════════════════ -->


    <!-- ═══════════════════════════════════════════════════════════════════════
         ZONE 2: THE REVELATION
         Sections: The Gas Town Mental Model
         Conviction brief: "The reader feels the *click*. The ant/colony
         insight. This is the page's emotional peak."
         Density: MEDIUM spatial, HIGH semantic. This zone breathes because
         the insight needs room to land.
         Background: clearing (#FEF9F5) — clean, open, maximum breathing room
         ═══════════════════════════════════════════════════════════════════════ -->
    <div class="zone zone--clearing transition--breathing" id="gas-town-mental-model" style="padding-top: var(--space-20); padding-bottom: var(--space-16);">
      <section class="section" aria-label="The Gas Town Mental Model">
        <div class="section__meta">Section 02 · Revelation</div>
        <h2 class="section__title">The Gas Town Mental Model</h2>

        <h3>The Core Insight</h3>

        <!-- The fulcrum quote — the page's emotional peak
             This is the moment of mental model restructuring.
             Extra vertical space around it because the insight needs
             room to land. -->
        <blockquote class="pull-quote" style="margin-top: var(--space-10); margin-bottom: var(--space-10);">
          <p class="pull-quote__text">"Claude Code is the world's biggest fuckin' ant. Everyone is focused on making their ant run longer... when nature prefers colonies."</p>
        </blockquote>

        <p>This is the fundamental shift Gas Town represents:</p>

        <!-- Paradigm shift comparison — the contrast does the teaching
             Conviction brief: "asymmetric layout — the 'Traditional Approach'
             and 'Gas Town Approach' side by side"
             Old way is muted/constrained. New way is clean/forward.
             R4: two-column comparison for conceptual contrast -->
        <div class="paradigm-shift">
          <div class="paradigm-shift__column paradigm-shift__column--old">
            <div class="paradigm-shift__header">Traditional Approach</div>
            <div class="paradigm-shift__step">One Big Ant</div>
            <div class="paradigm-shift__connector">↓</div>
            <div class="paradigm-shift__step">Make it run longer</div>
            <div class="paradigm-shift__connector">↓</div>
            <div class="paradigm-shift__step">Context window limits everything</div>
            <div class="paradigm-shift__connector">↓</div>
            <div class="paradigm-shift__step">You are the programmer</div>
          </div>
          <div class="paradigm-shift__column paradigm-shift__column--new">
            <div class="paradigm-shift__header">Gas Town Approach</div>
            <div class="paradigm-shift__step">Ant Colony</div>
            <div class="paradigm-shift__connector">↓</div>
            <div class="paradigm-shift__step">Build the factory</div>
            <div class="paradigm-shift__connector">↓</div>
            <div class="paradigm-shift__step">Multiple workers share the load</div>
            <div class="paradigm-shift__connector">↓</div>
            <div class="paradigm-shift__step">You are the Product Manager</div>
          </div>
        </div>
        ...
        <blockquote class="pull-quote">
          <p class="pull-quote__text">"Gas Town is an Idea Compiler. You just make up features, design them, file the implementation plans, and sling work around to your polecats and crew."</p>
        </blockquote>

        <!-- R5: Setup-payoff arc → quote (principle) then concrete flow (proof) -->
        <div class="idea-compiler">
          <div class="idea-compiler__label">The Compilation Pipeline</div>
          <div class="idea-compiler__body">
            <div class="idea-compiler__flow">
              <span class="idea-compiler__key">Input:</span>
              <span class="idea-compiler__value">Your ideas and feature requests</span>
              <span class="idea-compiler__key">Output:</span>
              <span class="idea-compiler__value">Working code, shipped products</span>
            </div>
            <p class="idea-compiler__statement">You're not writing code anymore. You're managing a factory.</p>
          </div>
        </div>
        ...
        <ul class="reason-list">
          <li>
            <span class="reason-list__marker">Post-apocalyptic resourcefulness</span>
            <span class="reason-list__detail">— Work with chaos, not against it</span>
          </li>
          <li>
            <span class="reason-list__marker">Clear role hierarchy</span>
            <span class="reason-list__detail">— Mayor, Deacon, Dogs, Workers</span>
          </li>
          <li>
            <span class="reason-list__marker">Survival-focused</span>
            <span class="reason-list__detail">— Throughput over perfection</span>
          </li>
          <li>
            <span class="reason-list__marker">Community-built</span>
            <span class="reason-list__detail">— Everyone contributes to the economy</span>
          </li>
        </ul>
  ...
  <!-- ═══════════════════════════════════════════════════════════════════════
       FOOTER — Mirrors header
       Mechanism #14: Footer Mirror (Dark + 3px Border-Top)
       ═══════════════════════════════════════════════════════════════════════ -->
  <footer class="page-footer" role="contentinfo">
    <div class="page-footer__inner">
      <span class="page-footer__id">Steve Yegge · Gas Town · Complete Extraction</span>
      <div class="page-footer__rule"></div>
      <span class="page-footer__id">Level 7 · Complexity Ladder</span>
    </div>
  </footer>
...
Rotation A, pass 2 of 8, subset 1 of 7 (global pass 2)
