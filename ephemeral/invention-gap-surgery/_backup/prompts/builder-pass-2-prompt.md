...
**Enrichment** adds density WITHIN existing zones — more CSS rules, more channels coordinated, more visual differentiation between zones. Enrichment makes zones RICHER without changing their boundaries.

**Preservation** keeps the structural skeleton from Pass 1 intact — zone boundaries, custom property names, content placement, responsive structure.

**Report these SEPARATELY in _pass-2-decisions.md.** If you changed a zone boundary, that's a structural change (preservation concern), not enrichment. If you added 3 CSS properties to an existing zone, that's enrichment.
