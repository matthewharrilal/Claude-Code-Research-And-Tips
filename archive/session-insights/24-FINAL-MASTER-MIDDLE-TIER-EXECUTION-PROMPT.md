# FINAL MASTER MIDDLE-TIER EXECUTION PROMPT
## For: Another Claude Instance to PLAN the Middle-Tier Page Build

**Synthesized from:** 5 research extractions (~5,300 lines), 4 adversarial reviews (~2,300 lines), 2 format references (~2,260 lines), and 290 files of design system research.

**Governing Principle:** Binary rules achieve 100% agent compliance. Judgment rules achieve ~0%.

**Date:** 2026-02-16
**Version:** FINAL (post-adversarial integration)
...
1. **Content selection** -- 800-1,200 word mixed-type content (prose + code + callouts + tables) in a domain DIFFERENT from Variant B's scientific calibration. **Content must be selected BEFORE reading Section 3 (Mechanism Vocabulary).**
...
4. **Build specification** -- complete HTML/CSS plan following the 10-step build workflow, with CONCRETE CSS values (not abstract descriptions)
...
**Judgment rules (these require interpretation -- err toward specific evidence over generic claims):**
...
## NON-NEGOTIABLE #1: Container Width
...
If you want a narrower aesthetic, use INTERNAL PADDING -- never reduce container width. This rule overrides ALL other layout considerations.
...
| 17 | NEVER use non-italic h3 headings | NONE -- ALL h3 elements in the rendered HTML MUST be italic. Set `h3 { font-style: italic; }` globally to catch all instances. |
...
| `--color-zone-dense` | `#FEFEFE` | Dense content zones (near-white) |
...
**Each mechanism belongs to EXACTLY ONE category (defined above). A mechanism cannot be counted toward multiple categories.**
...
.zone--dense     { background: var(--color-zone-dense);     } /* #FEFEFE near-white */
...
## 3.5 Middle-Tier Example Pairs (2-Mechanism Combinations)

These are EXAMPLE pairs that work at Middle-tier. The builder may create OTHER pairs as long as they are documented with shared semantic dimensions (B4.4). Middle-tier uses PAIRS (2 mechanisms reinforcing). Ceiling uses TRIPLES (3+ mechanisms reinforcing). Do NOT attempt triple-level combination at Middle tier.
...
**VALID example:** "#4 Spacing Compression + #7 Zone Background = both encode section importance (padding + background). Intro: sparse padding (80px) + light background. Detail: dense padding (32px) + near-white background."
...
**NOTE:** Do NOT default to CRESCENDO because most suggested domains (Section 9.4) are tutorial-like. Evaluate the ACTUAL structure of your selected content against all 4 patterns before choosing.
...
| Density range | Padding range across sections must span >= 32px (e.g., shallowest 64px, densest 32px) | B9+ |
...
**DOCUMENT in your plan:** "Container width 940-960px is HIGHEST priority. Narrowing is expressed via internal padding."
...
Rejection rationale must be for mechanisms you EVALUATED and DECIDED NOT TO DEPLOY. If you deploy 8 mechanisms, you should have evaluated all 18 and can reject 3+ of the remaining 10.

**Content-based, NOT catalog-based:**
- CORRECT: "This content has 4 code blocks showing progressive API complexity, so mechanism #17 (dark background, syntax highlighting) differentiates code zones from surrounding prose, reinforcing the CRESCENDO density pattern."
...
**PA-05 forward reference:** Your build plan must ensure PA-05a (>= 2 non-default layout elements), PA-05b (padding ratio >= 2.0x), PA-05c (identifiable visual hierarchy), PA-05d (>= 15% compositional CSS). See Section 5.3 for full definitions.
...
h3 {
  ...
  font-style: italic;   /* ALL h3 MUST be italic -- globally enforced */
  font-weight: 400;
  line-height: 1.4;
...
## 4.9 Plan Output Format (Two-Instance Handoff)

This prompt is for a PLANNING instance. The plan is handed to a separate BUILDING instance. The builder's continuation bias will anchor to whatever the plan starts with -- so the plan must start with ACTIONABLE STRUCTURE, not philosophical framing.

**Your plan MUST include these sections in this order:**

### 1. CONTENT
The full text (800-1,200 words) with section breaks marked. All content committed.

### 2. SECTION MAP
For each section: content summary, mechanism(s) deployed, padding value, background token, border treatment.

**WRONG:** "Deploy mechanism #5 for rhythm in the intro section."
**CORRECT:** "Section 1 (Intro): padding var(--space-16) [64px], background var(--color-zone-sparse), no border. Mechanism #5 sparse zone."

### 3. MECHANISM TABLE
Each deployed mechanism with: section location, CSS property, exact values.

### 4. TRANSITION TABLE
Each transition with: location (between which sections), type (SMOOTH/BRIDGE/BREATHING), CSS values.

### 5. REINFORCING PAIRS
2+ pairs with shared semantic dimension and CSS evidence for both members.

### 6. FRACTAL TABLE
Scale coverage table (Page + Component) with CSS evidence (from Step 8).

### 7. BUILDER BLOCKS
Copy Appendix B blocks 1-4 verbatim into the plan.

**The plan is NOT a discussion document.** It is an execution specification. No hedging ("we might consider"), no alternatives ("option A or option B"). Every decision must be MADE.
...
| 14 | Better than Variant B: richness | Middle deploys more mechanisms (Variant B = 7) AND mechanisms Variant B lacked (code blocks, header/footer, color encoding, grid, solid offset) |
| 15 | Better than Variant B: verdict | Clear improvement with 2+ specific PERCEPTUAL observations |
...
**Why Middle = 8-10:** The tier model is a fluency curriculum. Floor = deploy mechanisms independently (2-3 categories). Middle = deploy mechanisms with per-category breadth and reinforcing pairs (5 categories). The 8-10 range is the natural landing zone when content-structure mapping covers all 5 categories with mixed content. For detailed methodology rationale, see `ephemeral/doc-enrichment/02-tier-methodology.md`.
...
- Do NOT treat case studies as templates -- extract MECHANISMS, not implementations
...
**Key insight:** Variant B achieved 100% category coverage with only 7 mechanisms. Middle-tier must demonstrate improvement through MECHANISM DENSITY (more mechanisms per category) and MECHANISM DIVERSITY (deploying mechanisms Variant B lacked, especially code blocks #17, header/footer #13/#14, and color encoding #9). The advantage is not category count (both are 5/5) but technique saturation and the inclusion of mechanisms that prose-only content prevented Variant B from deploying.
...
**Note:** The CSS below shows MECHANISM-LEVEL patterns (vocabulary), not page-level templates. Use the structural PATTERN (border-left: 4px + 2-zone layout) but create YOUR class names, YOUR semantic mappings, YOUR content-specific adaptations. DO NOT copy class names like `.callout--info` verbatim if they don't match your content.

**Novelty note:** CSS blocks in Sections 4.3-4.7 and 7.4-7.5 are MECHANISM-LEVEL vocabulary. Reusing these CSS patterns is CORRECT behavior. Novelty assessment (D3.2) compares against SHOWCASE PAGES (DD-006, CD-006, OD-004), not against this prompt's CSS examples.
...
.section--dense {
  padding: var(--space-8);     /* 32px */
  background: var(--color-zone-dense);   /* #FEFEFE near-white */
  border: 1px solid var(--color-border);
}
.section--breathing {
...
**CRITICAL: Content selection is Step 1 of your planning process.** Select or create content BEFORE reading Section 3 (Mechanism Vocabulary). Content must be committed BEFORE mechanisms are considered. This prevents reverse-fitting (subconsciously writing content that fits pre-selected mechanisms).
...
| E2.2 | Better than Variant B = YES | Clear improvement, NOT marginal. At least 1 observation must be PERCEPTUAL ("zone differentiation creates visual sections that Variant B lacks") not PROCEDURAL ("more mechanism categories"). Procedural differences are expected BY CONSTRUCTION. |
...
**NOTE:** Novelty assessment (D3.1-D3.3) is performed AFTER the page is built and rendered, not during planning. The plan should document HOW novelty will be assessed, not perform the assessment itself.
...
| D3.2 CSS value overlap | For shared mechanisms: extract CSS values, compare against showcase pages | < 30% overlap | > 60% overlap |
...
| 8 | `design-system/compositional-core/components/merged-components.css` | 1,195 | Component CSS library (optional -- use if prompt-provided CSS is insufficient). |
...
## Tier 4: For Deeper Context (If Questions Arise)

| Topic | File |
|-------|------|
| Border grammar (combination rules) | `design-system/compositional-core/grammar/border-system.md` |
| Component inventory (confidence levels) | `design-system/compositional-core/components/component-inventory.md` |
| Research synthesis (337 findings) | `design-system/research/RESEARCH-SYNTHESIS.md` |
| Tier methodology (why these numbers) | `ephemeral/doc-enrichment/02-tier-methodology.md` |
| Density patterns (source definitions) | `design-system/specification/patterns/density-patterns.md` |

**The complete 290-file reference map is at `ephemeral/session-insights/22-middle-tier-file-reference-map.md`.**
...
*End of FINAL-MASTER-MIDDLE-TIER-EXECUTION-PROMPT.md. A Claude instance reading this document has everything needed to plan and execute the Middle-tier experiment. This version incorporates findings from 4 adversarial reviews resolving 5 critical gaps, 5 ambiguities, 4 contradictions, and 3 gaming vulnerabilities.*
